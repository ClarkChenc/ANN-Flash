...
void progressBar(int current, int total, std::chrono::steady_clock::time_point startTime, int barWidth = 50) {
  ...
  std::cout << "] " << std::setw(3) << std::setfill(' ') << int(progress * 100.0) << "% " << std::setw(2)
            << std::setfill('0') << hours << ":" << std::setw(2) << std::setfill('0') << minutes << ":"
...
template <typename T>
void WriteData(const std::string& file_path, std::vector<std::vector<T>>& results) {
