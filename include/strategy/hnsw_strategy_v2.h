...
#include "../../third_party/hnswlib/hnswalg_v2.h"

class HnswStrategy_V2: public SolveStrategy {
public:
    HnswStrategy_V2(std::string source_path, std::string query_path, std::string codebooks_path, std::string index_path):
        SolveStrategy(source_path, query_path, codebooks_path, index_path) {
    }

    void solve() {
        // Build HNSW index
        hnswlib::L2Space_V2 l2space(data_dim_, DIRECTION_NUM);
        hnswlib::HierarchicalNSW_V2<float> hnsw(&l2space, data_num_, M_, ef_construction_);
