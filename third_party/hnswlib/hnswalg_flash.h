...
class NeighborDataCache {
 ...
 public:
  ...
  NeighborDataCache(size_t size, size_t element_size) : max_size_(size), element_size_(element_size) {
    std::cout << "construct NeighborDataCache with size: " << size << ", element_size: " << element_size
              << std::endl;
  ...
  std::shared_ptr<data_t> get(tableint id) {
    ...
    count += 1;
    if (count % 1000 == 0) {
      std::cout << "thread_id: " << omp_get_thread_num()
                << ",hit rate: " << (stat_get_hit_count_ * 1.0 / stat_get_total_count_) << std::endl;
    }
...
class HierarchicalNSWFlash : public AlgorithmInterface<dist_t> {
 public:
  ...
  void set_dist_table(const void* data_point, tableint internal_id) {
    // labeltype return_label = getExternalLabel(internal_id);
    // memcpy(flash_data_dist_table_ + SUBVECTOR_NUM * CLUSTER_NUM * return_label, data_point, SUBVECTOR_NUM *
    // CLUSTER_NUM * sizeof(data_t));
    memcpy(flash_data_dist_table_ + SUBVECTOR_NUM * CLUSTER_NUM * internal_id, data_point,
           SUBVECTOR_NUM * CLUSTER_NUM * sizeof(data_t));
  }
  ...
  */
  void setLinksData(const void* data, size_t idx, tableint neighbor_id, int level) const {
    ...
    // uint8_t *currObj = (uint8_t *)data + (level == 0 ? offsetLinklistData0_ : offsetLinklistData_) -
    // sizeof(linklistsizeint) + block * VECTORS_PER_BLOCK * data_dim_; uint8_t *data_point = (uint8_t
  ...
  searchBaseLayer(tableint ep_id, const void* data_point, int layer) {
    ...
    uint16_t* encode_data_point =
        (uint16_t*)((char*)data_point + SUBVECTOR_NUM * CLUSTER_NUM * sizeof(data_t));
    ...
    if (!isMarkedDeleted(ep_id)) {
      ...
      dist_t dist = PqSdcL2Sqr(encode_data_point, getDataByInternalId(ep_id));
    ...
    while (!candidateSet.empty()) {
      ...
      std::vector<uint16_t> neighbors_data_vec(size * SUBVECTOR_NUM, 0);
      uint16_t* neighbors_data = neighbors_data_vec.data();

      for (int k = 0; k < size; ++k) {
        tableint neighbor_id = datal[k];
        uint16_t* neighbor_data = (uint16_t*)getDataByInternalId(neighbor_id);

        uint16_t* dst = neighbors_data + k * SUBVECTOR_NUM;
      ...
      for (size_t j = 0; j < size; j++) {
        ...
        dist_t dist1 = PqSdcL2Sqr(encode_data_point, currObj1);
  ...
  searchBaseLayerST(tableint ep_id,
    ...
    auto add_pace = [&](int candidate_dist) {
      ...
      pace.emplace(candidate_dist);
      if (max_heap.size() == 0 || *max_heap.begin() >= candidate_dist)
        max_heap.emplace(candidate_dist);
      else
        min_heap.emplace(candidate_dist);
    ...
    while (!candidate_set.empty()) {
      ...
      uint16_t* neighbors_data = (uint16_t*)alloca(size * SUBVECTOR_NUM * sizeof(uint16_t));
      tableint* datal = (tableint*)(data + 1);
...
#endif
        const uint16_t* neighbor_data = (uint16_t*)getDataByInternalId(neighbor_id);
...
#else
            fstdistfunc_(get_dist_table(second_pair.second), getDataByInternalId(curent_pair.second),
                         dist_func_param_);
#endif
      ...
      if (!is_cur_c_present) {
        if (sz_link_list_other < Mcurmax) {
          ...
          PqLinkL2Sqr(dist_list, get_dist_table(selectedNeighbors[idx]),
                      getLinksData(selectedNeighbors[idx], level), sz_link_list_other, level);
#endif
          // todo: 可以整体一起做
          for (size_t j = 0; j < sz_link_list_other; j++) {
#if defined(PQLINK_CALC) && !defined(SAVE_MEMORY)
    ...
    for (int layer = 0; layer <= elemLevel; layer++) {
      ...
      for (auto&& neigh : sNeigh) {
        ...
        for (auto&& cand : sCand) {
          ...
          dist_t distance = fstdistfunc_(get_dist_table(cand), getDataByInternalId(neigh), dist_func_param_);
  ...
  dist_t PqSdcL2Sqr(const void* Vec1, const void* Vec2) const {
    uint16_t* pVect1 = (uint16_t*)Vec1;  // distance encode
    uint16_t* pVect2 = (uint16_t*)Vec2;  // distance encode
