...
class HierarchicalNSWFlashV2 {
 ...
 public:
  HierarchicalNSWFlashV2(SpaceInterface<index_dist_t>* index_space,
  ...
  std::priority_queue<std::pair<index_dist_t, tableint>,
                      std::vector<std::pair<index_dist_t, tableint>>,
                      CompareByFirstLess>
  searchBaseLayer(tableint ep_id, const void* data_point, int layer) {
    ...
    std::priority_queue<std::pair<index_dist_t, tableint>, std::vector<std::pair<index_dist_t, tableint>>,
                        CompareByFirstLess>
        top_candidates;
    std::priority_queue<std::pair<index_dist_t, tableint>, std::vector<std::pair<index_dist_t, tableint>>,
                        CompareByFirstLess>
    ...
    if (!isMarkedDeleted(ep_id)) {
      index_dist_t dist =
          index_fstdistfunc_(data_point, getIndexDataByInternalId(ep_id), index_dist_func_param_);
...
//                    if (candidate_id == 0) continue;
  ...
  std::priority_queue<std::pair<search_dist_t, tableint>,
                      std::vector<std::pair<search_dist_t, tableint>>,
                      CompareByFirstLess>
  searchBaseLayerST(tableint ep_id,
    ...
    std::priority_queue<std::pair<search_dist_t, tableint>, std::vector<std::pair<search_dist_t, tableint>>,
                        CompareByFirstLess>
        top_candidates;
...
//                    if (candidate_id == 0) continue;
        ...
        if (!(visited_array[candidate_id] == visited_array_tag)) {
          ...
          if (flag_consider_candidate) {
            ...
            while (flag_remove_extra) {
              ...
              top_candidates.pop();
              if (!bare_bone_search && stop_condition) {
                stop_condition->remove_point_from_result(getExternalLabel(id), getSearchDataByInternalId(id),
                                                         dist);
                flag_remove_extra = stop_condition->should_remove_extra();
              } else {
                flag_remove_extra = top_candidates.size() > ef;
  ...
  void getNeighborsByHeuristic2(std::priority_queue<std::pair<index_dist_t, tableint>,
                                                    std::vector<std::pair<index_dist_t, tableint>>,
      ...
      for (std::pair<index_dist_t, tableint> second_pair : return_list) {
        index_dist_t curdist =
            index_fstdistfunc_(getIndexDataByInternalId(second_pair.second),
                               getIndexDataByInternalId(curent_pair.second), index_dist_func_param_);
    ...
    for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
      ...
      if (!is_cur_c_present) {
        if (sz_link_list_other < Mcurmax) {
          ...
          index_dist_t d_max =
              index_fstdistfunc_(getIndexDataByInternalId(cur_c),
                                 getIndexDataByInternalId(selectedNeighbors[idx]), index_dist_func_param_);
          // Heuristic:
          std::priority_queue<std::pair<index_dist_t, tableint>,
                              std::vector<std::pair<index_dist_t, tableint>>, CompareByFirstLess>
              candidates;
          candidates.emplace(d_max, cur_c);

          for (size_t j = 0; j < sz_link_list_other; j++) {
            candidates.emplace(
                index_fstdistfunc_(getIndexDataByInternalId(data[j]),
                                   getIndexDataByInternalId(selectedNeighbors[idx]), index_dist_func_param_),
                data[j]);
          }
    ...
    for (int layer = 0; layer <= elemLevel; layer++) {
      ...
      for (auto&& neigh : sNeigh) {
        ...
        for (auto&& cand : sCand) {
          ...
          index_dist_t distance = index_fstdistfunc_(getIndexDataByInternalId(cand),
                                                     getIndexDataByInternalId(neigh), index_dist_func_param_);
  ...
  void repairConnectionsForUpdate(const void* dataPoint,
    ...
    if (dataPointLevel < maxLevel) {
      index_dist_t curdist = index_fstdistfunc_(get_index_data(dataPoint), getIndexDataByInternalId(currObj),
      ...
      for (int level = maxLevel; level > dataPointLevel; level--) {
        ...
        while (changed) {
          ...
          for (int i = 0; i < size; i++) {
            ...
            index_dist_t d = index_fstdistfunc_(get_index_data(dataPoint), getIndexDataByInternalId(cand),
                                                index_dist_func_param_);
            if (d < curdist) {
              curdist = d;
              currObj = cand;
      ...
      if (filteredTopCandidates.size() > 0) {
        ...
        if (epDeleted) {
          filteredTopCandidates.emplace(
              index_fstdistfunc_(get_index_data(dataPoint), getIndexDataByInternalId(entryPointInternalId),
                                 index_dist_func_param_),
  ...
  tableint addPoint(const void* data_point, labeltype label, int level) {
    ...
    if ((signed)currObj != -1) {
      if (curlevel < maxlevelcopy) {
        index_dist_t curdist = index_fstdistfunc_(get_index_data(data_point),
                                                  getIndexDataByInternalId(currObj), index_dist_func_param_);
        for (int level = maxlevelcopy; level > curlevel; level--) {
          ...
          while (changed) {
            ...
            for (int i = 0; i < size; i++) {
              ...
              index_dist_t d = index_fstdistfunc_(get_index_data(data_point), getIndexDataByInternalId(cand),
                                                  index_dist_func_param_);
              if (d < curdist) {
                curdist = d;
                currObj = cand;
      ...
      for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
        ...
        std::priority_queue<std::pair<index_dist_t, tableint>, std::vector<std::pair<index_dist_t, tableint>>,
                            CompareByFirstLess>
            top_candidates = searchBaseLayer(currObj, get_index_data(data_point), level);

        if (epDeleted) {
          top_candidates.emplace(
              index_fstdistfunc_(get_index_data(data_point), getIndexDataByInternalId(enterpoint_copy),
                                 index_dist_func_param_),
              enterpoint_copy);
          if (top_candidates.size() > ef_construction_) top_candidates.pop();
        }
  ...
  searchKnn(const void* query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
    ...
    tableint currObj = enterpoint_node_;
    search_dist_t curdist =
        search_fstdistfunc_(query_data, getSearchDataByInternalId(enterpoint_node_), search_dist_func_param_);
  ...
  std::vector<std::pair<search_dist_t, labeltype>> searchStopConditionClosest(
    ...
    for (int level = maxlevel_; level > 0; level--) {
      ...
      while (changed) {
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (cand < 0 || cand > max_elements_) throw std::runtime_error("cand error");
          search_dist_t d =
              search_fstdistfunc_(query_data, getSearchDataByInternalId(cand), search_dist_func_param_);
          if (d < curdist) {
            curdist = d;
            currObj = cand;
            changed = true;
